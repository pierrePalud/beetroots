...
class ResultsKernelsHierarchical(ResultsUtil):
    ...
    def read_data(
        ...
        # u
        # list_type_u = np.zeros((self.N_run, self.effective_T))
        list_accepted_u = np.zeros((self.N_run, self.effective_T))
        list_log_proba_u = np.zeros((self.N_run, self.effective_T))
        ...
        for seed, mc_path in enumerate(list_chains_folders):
            with h5py.File(mc_path, "r") as f:
                ...
                list_log_proba_u[seed] = np.array(f["list_u_log_proba_accept_t"])
    ...
    def plot_accept_freq(
        ...
        for seed in range(self.N_run):
            ...
            if accepted_mtm.size > k_mm_mtm:
                ...
                filename += f"freq_accept_{self.model_name}_seed{seed}_MTM.PNG"
        ...
        for seed in range(self.N_run):
            ...
            if accepted_pmala.size > k_mm_mala:
                ...
                filename = f"{folder_path}/freq_accept_"
                filename += f"{self.model_name}_seed{seed}_PMALA.PNG"
                plt.savefig(filename, bbox_inches="tight")
                plt.close()
        ...
        for seed in range(self.N_run):
            idx_pmala = list_type[seed] == 1
            accepted_pmala = list_accepted_u[seed, idx_pmala]
            ...
            if accepted_pmala.size > k_mm_mala:
                ...
                filename = f"{folder_path}/freq_accept_"
                filename += f"{self.model_name}_seed{seed}_PMALA_u.PNG"
                plt.savefig(filename, bbox_inches="tight")
                plt.close()
    ...
    def plot_log_proba_accept(
        ...
        for seed in range(self.N_run):
            ...
            if list_log_proba_mtm.size > k_mm_mtm:
                ...
                filename += f"{self.model_name}_seed{seed}_MTM.PNG"
                plt.savefig(filename, bbox_inches="tight")
                plt.close()
        ...
        for seed in range(self.N_run):
            idx_pmala = list_type[seed] == 1
            list_log_proba_pmala = list_log_proba_u[seed, idx_pmala]
            ...
            if list_log_proba_pmala.size > k_mm_mala:
                ...
                nan_mean = np.nanmean(list_log_proba_mtm)
                nan_median = np.nanmedian(list_log_proba_mtm)
                title = f"PMALA on u: log proba accept avg: {nan_mean:.3e},"
                title += f"median: {nan_median:.3e}"
